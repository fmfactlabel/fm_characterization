export class FMFactLabel {
    ...
    updateProperties(data, containerId) {
        ...
        container.selectAll("g.property-row")
            .data(data, (d) => d.name)
            .join((enter) => {
            const row = enter.append("g").attr("class", "property-row");
            row.append("rect").attr("class", "indent-rect").attr("fill", "white");
            row.append("text").attr("class", "collapse-icon").attr("cursor", "pointer");
            row.append("text").attr("class", "prop-name").attr("cursor", "pointer");
            row.append("text").attr("class", "prop-value");
            row.append("text").attr("class", "prop-ratio");
            return row;
        })
            .each(function (d, i) {
            ...
            row.select(".bg-row")
                ...
                .attr("fill", self.showZebra && i % 2 !== 0 ? "#f8f9fc" : "transparent");
...
//# sourceMappingURL=FMFactLabel%20copy.js.map