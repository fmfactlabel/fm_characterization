...
async function loadConfiguration() {
    ...
    window.CONFIG = CONFIG; // Exponemos la configuración globalmente por si es necesaria en otros módulos
    ...
    if (fileURL) {
        ...
        if (CONFIG.is_flask) {
            ...
            try {
                ...
                const fmName = await engine.processFromURL(fileURL, (p, msg, details) => processBar.update(p, msg, details));
                processBar.update(100, "Fact Label generated!", "Analysis completed!");
                processBar.setState("success");
...
function setupPersistenceButtons(factLabel) {
    const buttons = [
        ...
        if (btn) {
            ...
            newBtn.addEventListener("click", (e) => {
                ...
                if (btnInfo.type === "export") {
                    factLabel.export(btnInfo.format);
                }
                else {
                    // Pasamos el engine que ya tienes en el scope de main.ts
                    factLabel.downloadSource(btnInfo.format, engine);
                }
            });
        }
    });
