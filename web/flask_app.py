...
from fmfactlabel import FMCharacterization, DatasetCharacterization
...
def datasetUpload():
    light_fact_label = 'lightFactLabel' in flask.request.form
    zip_file = flask.request.files.get('inputDataset')
    ...
    # Pasamos la lógica de creación como una función lambda
    return process_characterization_stream(
        fetch_coro_func=lambda on_p: DatasetCharacterization.from_path_async(filename, light_fact_label, on_progress=on_p),
        form_data=form_data,
        cleanup_path=filename
    )
...
def process_characterization_stream_for_dataset(fetch_coro_func, form_data=None, cleanup_path=None):
    """
    Función unificada para procesar la caracterización y devolver una Response SSE.
    :param fetch_coro_func: Función que recibe (on_prog) y devuelve la corrutina de FMCharacterization.
    :param form_data: Diccionario con metadatos extra para sobreescribir.
    ...
    def background_task():
        ...
        try:
            ...
            # 2. Aplicar Metadatos del formulario si existen
            if form_data:
                if form_data.get('name'): char['metadata'][0]['value'] = form_data['name']
                char['metadata'][1]['value'] = form_data.get('author')
                char['metadata'][2]['value'] = form_data.get('description')
                char['metadata'][3]['value'] = form_data.get('year')
                char['metadata'][4]['value'] = form_data.get('reference')
                char['metadata'][5]['value'] = form_data.get('keywords')
                char['metadata'][6]['value'] = form_data.get('domain')

            name = char['metadata'][0]['value']
            txt_content = str(char)
            ...
            json_path = temp_dir / f"{name}.json"
            output_filepath = str(f'{char["metadata"][0]["value"]}.json')
            with open(output_filepath, 'w', encoding='utf-8') as output_file:
                    json.dump(char, output_file, indent=4, ensure_ascii=False)
            delete_file_later(json_path)
