...
class FMProperties(Enum):
    ...
    CONFIGURATIONS = FMProperty('Configurations', 'Number of configurations represented by the feature model. This considers Non-Boolean features and multifeatures. If <=, >=, or ~ is shown, the number represents an estimation.', None)
    BOOLEAN_CONFIGURATIONS = FMProperty('Boolean configurations', 'Number of configurations represented by the feature model considering all features as Boolean features. If <= is shown, the number represents an upper estimation bound.', CONFIGURATIONS)
    UNBOUNDED_FEATURES = FMProperty('Unbounded features', 'Features that can take infinite (unbounded) values and lead to infinite configuration spaces. The ratio is based on the total number of features.', CONFIGURATIONS)
