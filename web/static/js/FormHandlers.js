...
export async function handleFMSubmission(form, engine, onResult, onProgress) {
    ...
    if (onProgress)
        onProgress(5, "Processing feature model...", "Preparing virtual file system");
...
export async function handleDatasetSubmission(form, engine, onResult, onProgress) {
    const formData = new FormData(form);
    const fileInput = form.querySelector('#inputDataset');
    const file = fileInput.files?.[0];
    if (!file) {
        alert("Please, select a file.");
        return;
    ...
    if (onProgress)
        onProgress(5, "Processing dataset...", "Preparing virtual file system");
    // 1. Escribir archivo en la memoria de Pyodide
    const arrayBuffer = await file.arrayBuffer();
    engine.writeFile(file.name, new Uint8Array(arrayBuffer));
    // 2. Extraer datos del formulario
